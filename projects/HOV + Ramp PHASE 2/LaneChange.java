...
public class LaneChange implements Constants
{
	...
	private double bsave=MAIN_BSAVE;     // maximum safe braking deceleration
	...
	public LaneChange (String lctype)
	{
		...
		{
			System.out.println("in Cstr LaneChange(''truck''): "+"db=" +db+" bsave="+bsave);
	...
	public boolean changeOK(Moveable me,  Moveable fOld, Moveable fNew, Moveable bNew)
	{
		...
		// efficient: if first cond. not satisfied,
		// the latter will not be evaluated (b_acc defined for efficiency)

		double bNew_acc; 
		//System.out.println("gapFront="+gapFront);
		//System.out.println("gapBack="+gapBack);
		if ((gapFront>gapMin) && (gapBack>gapMin) && (bNew_acc=bNew.model().calcAcc(bNew,me)) >- bsave)
		{

			 

			double my_adv  = me.model().calcAcc(me,fNew) - me.model().calcAcc(me,fOld)+ ((me.lane()==LEFT) ? 1 : -1) * biasRight;
